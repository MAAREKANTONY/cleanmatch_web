...
class BigQueryServiceQueryTests(TestCase):
    ...
    @patch('django.conf.settings.BIGQUERY_CREDENTIALS_FILE', '')
    @patch('django.conf.settings.BIGQUERY_OUTPUT_TABLE', 'segmented')
    @patch('django.conf.settings.BIGQUERY_INSERT_BATCH_SIZE', 2)
    @patch('bigquery.client.bigquery.Client')
    def test_write_segmented_rows_in_batches_chunks_payload(self, client_cls):
        client = MagicMock()
        client_cls.return_value = client
        client.get_dataset.return_value = object()
        client.get_table.return_value = object()
        ...
        service = BigQueryService()
        count = service.write_segmented_rows_in_batches(None, [
            {'google_place_id': '1'},
            {'google_place_id': '2'},
            {'google_place_id': '3'},
        ])
        ...
        first_batch = client.insert_rows_json.call_args_list[0].args[1]
        second_batch = client.insert_rows_json.call_args_list[1].args[1]
        self.assertEqual(len(first_batch), 2)
        self.assertEqual(len(second_batch), 1)

    def test_build_segmented_row_uses_datetime_format_without_timezone_by_default(self):
        row = BigQueryService.build_segmented_row(google_place_id='1', segments=['A'], process_id='job1')
        self.assertIn('created_at', row)
        self.assertNotIn('+00:00', row['created_at'])
        self.assertIn(' ', row['created_at'])

    def test_build_segmented_row_can_emit_timestamp_with_timezone(self):
        row = BigQueryService.build_segmented_row(google_place_id='1', segments=['A'], process_id='job1', created_at_mode='TIMESTAMP')
        self.assertIn('+00:00', row['created_at'])
